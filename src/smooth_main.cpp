...
void print_usage(const char* prog) {
    std::cerr
        ...
        << "Local beta-binomial smoothing for all samples in the TSV (streaming).\n"
        << "Observed methylation and coverage are smoothed; missing values are left unchanged.\n"
        ...
        << "  --counts-cov  Smooth counts and coverage instead of fraction (default: fraction)\n\n"
        << "Haplotype mode (default): writes {id}.hap{1,2}_frac_smoothed.\n"
        << "Sample mode (--sample): writes {id}.frac_smoothed "
...
int main(int argc, char* argv[]) {
    ...
    opts.operation = impute_methylation::ProcessOperation::Smooth;
    std::string input, output;
    if (!parse_args(argc, argv, opts, input, output)) return 1;

    try {
        impute_methylation::stream_beta_binomial_smooth_all(input, output, opts);
    } catch (const std::exception& e) {
        std::cerr << e.what() << '\n';
        return 1;
    }

    const char* smooth_mode =
        opts.mode == impute_methylation::ImputeMode::CountsCov ? "counts/cov" : "fraction";
    std::cerr << "Wrote " << output << " ("
              << (opts.sample_mode ? "sample" : "haplotype") << " mode, smooth=" << smooth_mode
